...
1. **Using `Class.forName().newInstance()` (Reflection)**
2. **Using `clone()` Method**
3. **Using Deserialization**
4. **Using Factory Methods (e.g., in Frameworks and APIs)**
...
1. An object is **serialized** (converted to a byte stream) and saved to a file. 
2. The object is later **deserialized** (read back from the file) using the `ObjectInputStream` class.
...
MyClass newObject = (MyClass) ois.readObject();
```
...
| **`Spring`**  | `ApplicationContext context = new ...(); // (Internal 'new' is used)` Object bean = context.getBean("myBean");` | Frameworks like Spring use factories and reflection to manage and return object instances (beans).          |
